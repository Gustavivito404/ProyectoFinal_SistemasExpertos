...
for cliente in clientes:
    ...
    INSERT OR REPLACE INTO clientes (
        id_cliente, nombre, tipo, num_operaciones, cliente_frecuente,
        fecha_ultimo_servicio, bomba_electrica, bomba_piloto, bomba_combustion,
        tablero_control_propio, rfc, direccion
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        cliente["id_cliente"],
        cliente["nombre"],
...
for producto in productos:
    ...
    INSERT OR REPLACE INTO productos (
        id_producto, nombre, categoria, subcategoria, potencia_hp, precio,
        stock, stock_minimo, instalable, equipo_critico, requiere_mantenimiento,
        precio_variable, control_stock, requiere_diseno_tecnico
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
for servicio in servicios:
    ...
    INSERT OR REPLACE INTO servicios (
        id_servicio, nombre, tipo_servicio, costo_base, duracion_dias_base,
        nivel_validacion, considera_ultimo_servicio, requiere_revision_previa,
        observaciones_base
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
...
for regla in reglas:
    ...
    INSERT OR REPLACE INTO reglas_inferencia (
        id_regla, nombre, categoria, prioridad, agente_responsable,
        condiciones, acciones, explicacion
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        regla["id_regla"],
        regla["nombre"],
...
for grupo_alias in alias_productos:
    for alias in grupo_alias.get("alias", []):
        cursor.execute("""
        INSERT INTO alias_productos (id_producto, nombre_producto, alias)
        VALUES (?, ?, ?)
        """, (
...
for usuario in usuarios:
    ...
    INSERT OR REPLACE INTO usuarios_internos (
        id_usuario, nombre, rol, contrasena, nivel_validacion,
        puede_validar, puede_autorizar_servicios, requiere_supervision_tecnica
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        usuario["id_usuario"],
        usuario["nombre"],
